  ...
  if (!post) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50">
        ...
        <Link to="/resources" className="text-emerald-600 hover:underline flex items-center gap-2">
  ...
  return (
    <div className="bg-white min-h-screen pb-20">
      ...
      <div className="max-w-3xl mx-auto px-6 py-12">
        <Link to="/resources" className="inline-flex items-center gap-2 text-slate-500 hover:text-emerald-600 transition mb-8 text-sm font-medium">
            <ArrowLeft size={16} /> Retour aux articles
        </Link>

        <div className="prose prose-lg prose-slate prose-headings:font-serif prose-a:text-emerald-600 mx-auto">
        ...
        <div className="mt-16 pt-8 border-t border-slate-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {BLOG_POSTS.filter(p => p.id !== post.id).slice(0, 2).map(related => (
                    <Link key={related.id} to={`/blog/${related.slug}`} className="group block bg-slate-50 rounded-xl p-4 border border-slate-100 hover:border-emerald-200 transition">
                        <h4 className="font-bold text-slate-900 group-hover:text-emerald-600 transition">{related.title}</h4>
